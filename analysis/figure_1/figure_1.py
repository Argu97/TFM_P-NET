...
def plot_confusion_matrix_all(ax):
    base_dir = join(PROSTATE_LOG_PATH, 'TFM')
    models_base_dir = join(base_dir, 'prueba')
...
models = ['RBF Support Vector Machine ', 'Logistic Regression']
n = len(models) + 1

all_models_dict = {}

base_dir = PROSTATE_LOG_PATH
models_base_dir = join(base_dir, 'TFM/prueba')
for i, m in enumerate(models):
    df = pd.read_csv(join(models_base_dir, m + '_ALL_testing.csv'), sep=',', index_col=0, header=[0, 1])
    all_models_dict[m] = df

pnet_base_dir = join(base_dir, 'TFM/prueba')
