...
relations_file_name = 'relationships_GO.txt'
pathway_names = 'Pathways_GO.txt'
pathway_genes = 'a.gmt'
...
class Reactome():
    ...
    def load_names(self):
        ...
        df.columns = ['reactome_id', 'pathway_name']
        return df

    def load_genes(self):
        filename = join(reactome_base_dir, pathway_genes)
        gmt = GMT()
        df = gmt.load_data(filename, pathway_col=0, genes_col=1)
        return df
...
class ReactomeNetwork():
    ...
    def get_reactome_networkx(self):
        ...
        human_hierarchy = hierarchy[hierarchy['child'].str.contains('GO')]
        net = nx.from_pandas_edgelist(human_hierarchy, 'child', 'parent', create_using=nx.DiGraph())
        net.name = 'reactome'

        # add root node
        roots = [n for n, d in net.out_degree() if d == 0]
        root_node = 'root'
