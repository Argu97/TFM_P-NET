...
cnv_filename = 'prueba.txt'
response_filename = 'prueba_response.txt'
gene_important_mutations_only = 'prueba.txt'
...
class DataTFM():
    ...
    def get_train_validate_test(self):
        ...
        #x_train_test, x_validate, y_train_test, y_validate = train_test_split(x, y, test_size=0.1, random_state=42)
        #x_train, x_test, y_train, y_test = train_test_split(x_train_test, y_train_test, test_size=0.15, random_state=42)
        
        ind_train = (pd.read_csv(data_path+'/prostate/splits/train_{}.csv'.format('0'), delimiter=',',header = None))[0].to_list()
        ind_test = (pd.read_csv(data_path+'/prostate/splits/test_{}.csv'.format('0'), delimiter=',',header = None))[0].to_list()
        ind_validate = (pd.read_csv(data_path+'/prostate/splits/validate_{}.csv'.format('0'), delimiter=',',header = None))[0].to_list()
